...
namespace TrainingCenter.Services
{
    public class ReportService : IReportService
    {
        ...
        public GeneralResponseDto<List<AvailableTrackResponse>> GetAvailableTracks()
        {
            ...
            var avaliableTracks = query.Select(t => new AvailableTrackResponse()
            {
                TrackId = t.TrackId,
                Title = t.Title,
                Capacity = t.Capacity,
        ...
        public GeneralResponseDto<List<UnpaidEnrollmentResponse>> GetUnpaidEnrollments()
        {
            ...
            var query = context.Enrollments.Where(e => e.Payments.Any(p => p.PaymentStatus == PaymentStatus.Pending || p.PaymentStatus == PaymentStatus.PartiallyPaid)).AsQueryable();
            ...
            if (currentUserService.Role == Role.Instructor)
            {
                ...
                query = query.Where(e => e.TrainingTrack.InstructorId == instructorId.Value);
            }


            var data = query.Select(e => new UnpaidEnrollmentResponse
            {
            ...
            responseDto.Success = true;
            responseDto.Message = "Unpaid enrollments retrieved successfully";
        ...
        {
            var query = context.Enrollments.Where(e => e.Status == EnrollmentStatus.Active).AsQueryable();
        ...
        public GeneralResponseDto<List<TrackCapacityResponse>> GetTrackCapacity()
        {
            ...
            if (currentUserService.Role == Role.Instructor)
            {
                ...
                if (instructorId == null)
                {
                    return new GeneralResponseDto<List<TrackCapacityResponse>>
                    {
                        Success = false,
                        Message = "Instructor profile not found.",
                        ErrorType = ErrorType.NotFound
            ...
            var data = query.Select(t => new TrackCapacityResponse
            {
                TrackId = t.TrackId,
                TrackName = t.Title,
                Capacity = t.Capacity,
        ...
        {
            GeneralResponseDto<TrackProgressResponse> response = new();
            ...
            {
                response.Success = false;
                response.Message = "You are not allowed to view progress of another instructor's track.";
                response.ErrorType = ErrorType.Forbidden;
                return response;
            }

            var enrollments = context.Enrollments.Where(e => e.TrainingTrackId == id).Select(e => e.ProgressPercentage).ToList();

            var result = new TrackProgressResponse
            {
                TrackId = track.TrackId,
                TrackTitle = track.Title,
                TotalStudents = enrollments.Count,
                AverageProgress = enrollments.Any() ? (decimal) enrollments.Average(): 0
            };

            response.Success = true;
            response.Message = "Track progress retrieved successfully.";
