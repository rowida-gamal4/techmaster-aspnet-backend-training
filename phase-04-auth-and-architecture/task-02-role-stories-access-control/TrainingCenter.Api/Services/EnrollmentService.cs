...
namespace TrainingCenter.Services
{
    public class EnrollmentService : IEnrollmentService
    {
        ...
        public EnrollmentService(AppDbContext context, ICurrentUserService currentUserService)
        {
        ...
        public GeneralResponseDto<EnrollmentDetailsResponse> CreateEnrollment(CreateEnrollmentRequest request)
        {
            ...
            if (currentUserService.Role == Role.Student)
            {
                ...
                if (request.StudentId != currentStudentId.Value)
                {
                    response.Success = false;
                    response.Message = "You can not create enrollment for another student.";
            ...
            {
                response.Success = false;
                response.Message = "Enrollment failed to be created";
                response.ErrorType = ErrorType.Conflict;
                return response;
            }
        ...
        public GeneralResponseDto<List<EnrollmentDetailsResponse>> GetAllEnrollments(EnrollmentStatus? status, int? trackId, int? studentId, string? paymentStatus)
        {
            ...
            if (currentUserService.Role == Role.Student)
            {
                ...
                returnedEnrollments = returnedEnrollments.Where(e => e.StudentId == currentStudentId.Value);
            }
            //instructor 
            if (currentUserService.Role == Role.Instructor)
            {
                ...
                returnedEnrollments = returnedEnrollments.Where(e => e.TrainingTrack.InstructorId == currentInstructorId.Value);
            }
            ...
            if (!string.IsNullOrEmpty(paymentStatus))
            {
                ...
                returnedEnrollments = returnedEnrollments.Where(e => e.Payments.Any(p => p.PaymentStatus == statusEnum));
            }
        ...
        public GeneralResponseDto<EnrollmentDetailsResponse> GetEnrollmentById(int id)
        {
            ...
            if (currentUserService.Role == Role.Student)
            {
                ...
                if (enrollment.StudentId != currentStudentId.Value)
                {
                    responseDto.Success = false;
                    responseDto.Message = "You are not allowed to view this enrollment.";
            ...
            if (currentUserService.Role == Role.Instructor)
            {
                ...
                {
                    responseDto.Success = false;
                    responseDto.Message = "Instructor profile not found.";
                    responseDto.ErrorType = ErrorType.NotFound;
                    return responseDto;
                }

                if (enrollment.TrainingTrack.InstructorId != currentInstructorId.Value)
                {
                    responseDto.Success = false;
                    responseDto.Message = "You are not allowed to view this enrollment.";
                    responseDto.ErrorType = ErrorType.Forbidden;
        ...
        public GeneralResponseDto<bool> UpdateEnrollmentStatus(int id, UpdateEnrollmentStatusRequest request)
        {
            ...
            else
            {
                ...
                {
                    responseDto.Success = false;
                    responseDto.Message = "Completed enrollment cannot be modified.";
        ...
        public GeneralResponseDto<StudentEnrollmentHistoryResponse> GetStudentEnrollmentHistory(int id)
        {
            ...
            if (currentUserService.Role == Role.Student)
            {
                ...
                if (id != currentStudentId.Value)
                {
                    response.Success = false;
                    response.Message = "You are not allowed to view other student's enrollments.";
                    response.ErrorType = ErrorType.Forbidden;
                    return response;
                }
            }
        ...
        {
            GeneralResponseDto<TrackStudentsResponse> response = new();

            var track = context.TrainingTracks.FirstOrDefault(t => t.TrackId == id && !t.IsDeleted);
            if (track is null)
            {
                response.Success = false;
                response.Message = "Track not found";
            ...
            if (currentUserService.Role == Role.Instructor)
            {
                ...
                {
                    response.Success = false;
                    response.Message = "You are not allowed to view students of another instructor's track.";
                    response.ErrorType = ErrorType.Forbidden;
                    return response;
                }
            }

            var trackStudents = context.TrainingTracks.Where(t => t.TrackId == id).Select(t => new TrackStudentsResponse
                {
                    TrackId = t.TrackId,
                    TrackTitle = t.Title,
            ...
            response.Success = true;
            response.Message = "Track students retrieved successfully";
            response.Data = trackStudents;
